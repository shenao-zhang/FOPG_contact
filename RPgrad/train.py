    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            if self.done:
                ...
                actor_loss = -self.episode_reward / self.env._max_episode_steps
                self.logger.log('train_actor/loss', actor_loss, self.step)
                self.logger.log('train_actor/entropy', -self.first_log_pi.mean(), self.step)
                ...
                self.agent.temp.update(self.first_log_pi, self.logger, self.step)
            ...
            """
            _, action, log_pi = self.agent.actor(obs)
            if self.episode_step == 0:
                self.first_log_pi = log_pi
            # run training update
...
#              else 0.
            self.episode_reward += (reward - self.agent.temp.alpha.detach() * log_pi) * \
                                   (self.agent.discount ** self.episode_step)
